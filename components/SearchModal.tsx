...
const SearchItem = ({ coin, onSelect, isActiveName }: SearchItemProps) => {
  ...
  const price = isSearchCoin
    ? (coin as SearchCoin).data?.price
    : (coin as TrendingCoin['item']).data.price;
  ...
  return (
    ...
    >
      ...
      <p className='coin-price'>{price === undefined ? '—' : formatCurrency(price)}</p>
...
export const SearchModal = ({
  ...
  >(
    ...
    async ([, query]) => {
      const response = await fetch(`/api/coins/search?query=${encodeURIComponent(query as string)}`);

      if (!response.ok) throw new Error('Unable to search coins');

      return response.json() as Promise<SearchCoin[]>;
    },
    {
